...
if (true) {
    ...
    $base = "WITH Paginado AS ( 
		SELECT ROW_NUMBER() OVER(ORDER BY id ASC) AS linha, tbPesquisasWorkshop.* FROM tbPesquisasWorkshop) 
		SELECT Paginado.* FROM Paginado ";
    ...
    $base = "$base $termo ";
    $base = str_replace("Paginado.*", "count(*) as total ", $base);
